	...
	if (!(Monst->_mFlags & MFLAG_GOLEM)) {
		for (pnum = 0; pnum < MAX_PLRS; pnum++) {
			if (!plr[pnum].data.plractive || level.currlevel != plr[pnum].data.plrlevel || plr[pnum].data._pLvlChanging || (plr[pnum].data._pHitPoints == 0 && gbMaxPlayers != 1))
				continue;
			if (grid[Monst->_mx][Monst->_my].dTransVal == grid[plr[pnum].data._px][plr[pnum].data._py].dTransVal)
				sameroom = TRUE;
			else
				sameroom = FALSE;
			if (abs(Monst->_mx - plr[pnum].data._px) > abs(Monst->_my - plr[pnum].data._py))
				dist = Monst->_mx - plr[pnum].data._px;
			else
				dist = Monst->_my - plr[pnum].data._py;
			dist = abs(dist);
...
void M_StartHit(int i, int pnum, int dam)
{
	...
	if (monster[i].MType->mtype >= MT_SNEAK && monster[i].MType->mtype <= MT_ILLWEAV || dam >> 6 >= monster[i].mLevel + 3) {
		if (pnum >= 0) {
			...
			monster[i]._menemyx = plr[pnum].data._pfutx;
			monster[i]._menemyy = plr[pnum].data._pfuty;
...
void M_DiabloDeath(int i, BOOL sendmsg)
{
	...
	for (j = 0; j < nummonsters; j++) {
		...
		M_ClearSquares(k);
		grid[pmonster->_mx][pmonster->_my].dMonster = k + 1;
	}
...
void MonstStartKill(int i, int pnum, BOOL sendmsg)
{
	...
	M_ClearSquares(i);
	grid[Monst->_mx][Monst->_my].dMonster = i + 1;
...
void M_TryH2HHit(int i, int pnum, int Hit, int MinDam, int MaxDam)
{
	...
	if (plr[pnum].data._pHitPoints >> 6 <= 0 || plr[pnum].data._pInvincible || plr[pnum].data._pSpellFlags & 1)
		return;
	dx = abs(monster[i]._mx - plr[pnum].data._px);
	dy = abs(monster[i]._my - plr[pnum].data._py);
	if (dx >= 2 || dy >= 2)
		return;
...
#endif
	hit = Hit
	    + 2 * (monster[i].mLevel - plr[pnum].data._pLevel)
	    + 30
	...
	if (hper >= hit)
		return;
	if (blkper < blk) {
		plr[pnum].StartPlrBlock(GetDirection(plr[pnum].data._px, plr[pnum].data._py, monster[i]._mx, monster[i]._my));
		return;
	}
	...
	if (monster[i]._mFlags & MFLAG_KNOCKBACK) {
		...
		newx = plr[pnum].data._px + offset_x[monster[i]._mdir];
		newy = plr[pnum].data._py + offset_y[monster[i]._mdir];
...
void M_Teleport(int i)
{
	...
	if (tren) {
		M_ClearSquares(i);
		grid[Monst->_mx][Monst->_my].dMonster = 0;
		grid[x][y].dMonster = i + 1;
		Monst->_moldx = x;
		Monst->_moldy = y;
...
void PrepDoEnding()
{
	...
	killLevel = &plr.local().data.pDiabloKillLevel;
	newKillLevel = gnDifficulty + 1;
	if (*killLevel > newKillLevel)
		newKillLevel = *killLevel;
	plr.local().data.pDiabloKillLevel = newKillLevel;
		...
		if (grid[mx][my].dLight != lightmax) {
			...
			if (Monst->_mgoal == MGOAL_RETREAT) {
				if (Monst->_mFlags & MFLAG_TARGETS_MONSTER)
					md = GetDirection(Monst->_mx, Monst->_my, plr[Monst->_menemy].data._pownerx, plr[Monst->_menemy].data._pownery);
				md = opposite[md];
		...
		for (y = -rad; y <= rad; y++) {
			for (x = -rad; x <= rad; x++) {
				if (y >= 0 && y < MAXDUNY && x >= 0 && x < MAXDUNX) {
					m = grid[x + Monst->_mx][y + Monst->_my].dMonster;
					if (m > 0) {
						m--;
...
void MAI_Round(int i, BOOL special)
{
	...
	if (Monst->_mmode == MM_STAND && Monst->_msquelch != 0) {
		...
		if (Monst->_msquelch < UCHAR_MAX)
			MonstCheckDoors(i);
		v = random_(114, 100);
		if ((abs(mx) >= 2 || abs(my) >= 2) && Monst->_msquelch == UCHAR_MAX && grid[Monst->_mx][Monst->_my].dTransVal == grid[fx][fy].dTransVal) {
			if (Monst->_mgoal == MGOAL_MOVE || (abs(mx) >= 4 || abs(my) >= 4) && random_(115, 4) == 0) {
				...
				if (abs(mx) > abs(my))
					...
					dist = abs(my);
				if (Monst->_mgoalvar1++ >= 2 * dist && DirOK(i, md) || grid[Monst->_mx][Monst->_my].dTransVal != grid[fx][fy].dTransVal) {
					Monst->_mgoal = MGOAL_NORMAL;
...
void MAI_Scav(int i)
{
	...
	if (Monst->_mgoal == MGOAL_HEALING && Monst->_mgoalvar3 != 0) {
		Monst->_mgoalvar3--;
		if (grid[Monst->_mx][Monst->_my].dDead) {
			M_StartEat(i);
			if (!(Monst->_mFlags & MFLAG_NOHEAL))
				...
				if (random_(120, 2) != 0) {
					for (y = -4; y <= 4 && !done; y++) {
						for (x = -4; x <= 4 && !done; x++) {
							...
							if (y < 0 || y >= MAXDUNY || x < 0 || x >= MAXDUNX)
								continue;
							done = grid[Monst->_mx + x][Monst->_my + y].dDead != 0
							    && LineClearF(
							           CheckNoSolid,
							           Monst->_mx,
							           Monst->_my,
...
void MAI_Golum(int i)
{
	...
	ok = M_CallWalk(i, plr[i].data._pdir);
	if (!ok) {
		md = (md - 1) & 7;
		for (j = 0; j < 8 && !ok; j++) {
			md = (md + 1) & 7;
	...
	if (Monst->_mmode == MM_STAND && Monst->_msquelch != 0) {
		...
		if (abs(mx) >= 2 || abs(my) >= 2) {
			if (Monst->_mgoal == MGOAL_MOVE || (abs(mx) >= 5 || abs(my) >= 5) && random_(132, 4) != 0) {
				...
				if (Monst->_mgoalvar1++ >= 2 * dist || grid[Monst->_mx][Monst->_my].dTransVal != grid[fx][fy].dTransVal) {
					Monst->_mgoal = MGOAL_NORMAL;
				} else if (!M_RoundWalk(i, md, &Monst->_mgoalvar2)) {
					M_StartDelay(i, random_(125, 10) + 10);
				}
			...
			if ((abs(mx) >= 5 || abs(my) >= 5)
				...
				if (AddMissile(Monst->_mx, Monst->_my, fx, fy, md, MIS_RHINO, Monst->_menemy, i, 0, 0) != -1) {
					if (Monst->MData->snd_special)
						PlayEffect(i, 3);
					Monst->_mmode = MM_CHARGE;
					grid[Monst->_mx][Monst->_my].dMonster = -1 - i;
				}
			} else {
				if (abs(mx) >= 2 || abs(my) >= 2) {
					v = random_(134, 100);
...
void MAI_Zhar(int i)
{
	...
	my = Monst->_my;
	md = M_GetDir(i);
	if (Monst->mtalkmsg == TEXT_ZHAR1 && !(grid[mx][my].dFlags & BFLAG_VISIBLE) && Monst->_mgoal == MGOAL_TALKING) {
...
void MAI_SnotSpil(int i)
{
	...
	if (Monst->mtalkmsg == TEXT_BANNER10 && !(grid[mx][my].dFlags & BFLAG_VISIBLE) && Monst->_mgoal == MGOAL_TALKING) {
...
void MAI_Lazurus(int i)
{
	...
	if (grid[mx][my].dFlags & BFLAG_VISIBLE) {
		if (gbMaxPlayers == 1) {
			if (Monst->mtalkmsg == TEXT_VILE13 && Monst->_mgoal == MGOAL_INQUIRING && plr.local().data._px == TEXT_VILE13 && plr.local().data._py == 46) {
...
void MAI_Lachdanan(int i)
{
	...
	md = M_GetDir(i);
#ifndef SPAWN
	if (Monst->mtalkmsg == TEXT_VEIL9 && !(grid[_mx][_my].dFlags & BFLAG_VISIBLE) && monster[i]._mgoal == MGOAL_TALKING) {
	...
	if (!(Monst->_mFlags & MFLAG_TARGETS_MONSTER)) {
		...
		if (grid[oldx][oldy].dPlayer > 0) {
			if (Monst->MType->mtype != MT_GLOOM && (Monst->MType->mtype < MT_INCIN || Monst->MType->mtype > MT_HELLBURN)) {
				M_TryH2HHit(m, grid[oldx][oldy].dPlayer - 1, 500, Monst->mMinDamage2, Monst->mMaxDamage2);
		...
		if (grid[oldx][oldy].dMonster > 0) {
			if (Monst->MType->mtype != MT_GLOOM && (Monst->MType->mtype < MT_INCIN || Monst->MType->mtype > MT_HELLBURN)) {
				M_TryM2MHit(m, grid[oldx][oldy].dMonster - 1, 500, Monst->mMinDamage2, Monst->mMaxDamage2);
...
void SpawnGolum(int i, int x, int y, int mi)
{
	...
	monster[i]._mmaxhp = 2 * (320 * missile[mi]._mispllvl + plr[i].data._pMaxMana / 3);
	monster[i]._mhitpoints = monster[i]._mmaxhp;
	monster[i].mHit = 5 * (missile[mi]._mispllvl + 8) + 2 * plr[i].data._pLevel;
...
void decode_enemy(int m, int enemy)
{
	if (enemy < MAX_PLRS) {
		...
		monster[m]._menemyx = plr[enemy].data._pfutx;
		monster[m]._menemyy = plr[enemy].data._pfuty;
	} else {
		monster[m]._mFlags |= MFLAG_TARGETS_MONSTER;
		enemy -= MAX_PLRS;
		monster[m]._menemy = enemy;
