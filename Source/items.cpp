...
void AddInitItems()
{
	...
	for (j = 0; j < rnd; j++) {
		...
		if (random_(12, 2))
			GetItemAttrs(i, IDI_HEAL, lvl.currlevel);
		else
			GetItemAttrs(i, IDI_MANA, lvl.currlevel);
		item[i]._iCreateInfo = lvl.currlevel - 0x8000;
		SetupItem(i);
...
void GetItemAttrs(int i, int idata, int curlvl)
{
	int rndv;
...
void SpawnUnique(int uid, V2Di pos)
{
	...
	GetItemAttrs(ii, itype, lvl.currlevel);
...
void CreateItem(int uid, V2Di pos)
{
	...
	if (numitems < MAXITEMS) {
		...
		GetItemAttrs(ii, idx, lvl.currlevel);
		GetUniqueItem(ii, uid);
		SetupItem(ii);
...
void CreateRndItem(V2Di pos, bool onlygood, bool sendmsg, bool delta)
{
	...
	if (numitems < MAXITEMS) {
		...
		itemavail[0] = itemavail[MAXITEMS - numitems - 1];
		itemactive[numitems] = ii;
		SetupAllItems(ii, idx, GetRndSeed(), 2 * lvl.currlevel, 1, onlygood, FALSE, delta);
		if (sendmsg)
			NetSendCmdDItem(FALSE, ii);
		if (delta)
			DeltaAddItem(ii);
...
void CreateRndUseful(int pnum, V2Di pos, bool sendmsg)
{
	...
	if (numitems < MAXITEMS) {
		...
		itemavail[0] = itemavail[MAXITEMS - numitems - 1];
		itemactive[numitems] = ii;
		SetupAllUseful(ii, GetRndSeed(), lvl.currlevel);
		if (sendmsg) {
			NetSendCmdDItem(FALSE, ii);
		}
		numitems++;
...
void SpawnQuestItem(int itemid, V2Di pos, int randarea, int selflag)
{
	...
	if (numitems < MAXITEMS) {
		...
		item[i]._i = pos;
		grid.at(pos).setItem(i);
		GetItemAttrs(i, itemid, lvl.currlevel);
		SetupItem(i);
...
void SpawnRock()
{
	...
	if (ostand) {
		...
		GetItemAttrs(i, IDI_ROCK, lvl.currlevel);
		SetupItem(i);
		item[i]._iSelFlag = 2;
		item[i]._iPostDraw = TRUE;
...
void CreateSpellBook(V2Di pos, int ispell, bool sendmsg, bool delta)
{
	...
	if (numitems < MAXITEMS) {
		...
		itemactive[numitems] = ii;
		while (!done) {
			SetupAllItems(ii, idx, GetRndSeed(), 2 * lvl.currlevel, 1, TRUE, FALSE, delta);
